...
io.on("connection", (socket) => {
    ...
    socket.on("create", (roomName, userName, cb) => {
        ...
        io.to(roomName).emit("newMember", {roomName , userName , id:socket.id});
    ...
    socket.on("sendMessage", (message) => {
        ...
        const socketInRoom = getSocketInRoom(socket.room);
        const userNum = Number(message.userList);
        const user = socketInRoom[userNum];
        // usersInRoom[userNum] 는 Nan(user) 또는 사용자 닉네임이 나옴
        // socketInRoom[userNum] 는 보낼 사용자 id가 나옴
